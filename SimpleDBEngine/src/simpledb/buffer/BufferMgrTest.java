...
public class BufferMgrTest {
   public static void main(String[] args) throws Exception {
      ...
      buff[0] = bm.pin(new BlockId("blok0", 0));
      buff[1] = bm.pin(new BlockId("blok1", 1));
      buff[2] = bm.pin(new BlockId("blok2", 2));
      bm.unpin(buff[1]);
      buff[1] = null;
      buff[3] = bm.pin(new BlockId("blok0", 0)); // block 0 pinned twice
      buff[4] = bm.pin(new BlockId("blok1", 1)); // block 1 repinned
      System.out.println("Available buffers: " + bm.available());
      try {
         System.out.println("Attempting to pin block 3...");
         buff[5] = bm.pin(new BlockId("blok3", 3)); // will not work; no buffers left
      } catch (BufferAbortException e) {
         System.out.println("Exception: No available buffers\n");
      }
      bm.unpin(buff[2]);
      buff[2] = null;
      buff[5] = bm.pin(new BlockId("blok3", 3)); // now this works
